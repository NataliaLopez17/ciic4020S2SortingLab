...
public class QuickSort<E> extends AbstractSortingStrategy<E> {
	...
	/**
	 * Applies the divide and conquer strategy to sort using QuickSort algorithm. It
	 * works on the portion of the ArrayList from position first to position last
	 * (first..last).
	 * 
	...
	private void qs(ArrayList<E> data, int first, int last) {
		...
		int i = partitionList(data, first, last);
		qs(data, first, i - 1);
		qs(data, i, last);
	}
