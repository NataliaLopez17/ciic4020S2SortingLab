...
public class SelectionSort<E> extends AbstractSortingStrategy<E> {
	...
	public void sortList(ArrayList<E> dataSet) {
		 ...
		 * 
		 * To compare two values in dataSet, use something like: if
		 * (cmp.compare(dataSet.get(j), dataSet.get(sm)) < 0) To swap, use something
		 * like: SortingUtils.swapListElements(dataSet, sm, i); assuming that sm and i
		 * are indexes within dataSet.
		 */

		for (int i = 0; i < n - 1; i++) {
			for (int j = i + 1; j < n; j++) {
				if (cmp.compare(dataSet.get(j), dataSet.get(i)) < 0) {
